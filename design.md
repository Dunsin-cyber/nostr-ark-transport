...
* Be beneficial for horizontal scalability and microservice architecture, enabling high availability across multiple datacenters. Routing client traffic through relays rather than a fixed gRPC endpoint removes the connection level pinning that long lived gRPC streams create, so an ASP can scale out or fail over without forcing wallets to reconnect.
...
*Practical note.* Even under option B or C, the realistic relay set narrows to those that meet Ark's operational requirements (long lived connections, ephemeral event retention until consumed, adequate payload sizes, tolerance for the burst patterns of round coordination). NIP 47 is the closest precedent here: NWC ended up deployed mostly on provider operated relays plus a small curated set, not arbitrary public relays. Ark would likely inherit the same shape, so the rotation surface is "an opt in subset of relays meeting these requirements" rather than "all of Nostr." This bounds how strong the censorship resistance argument is in practice (see §7.1 and §8).
...
The reverse direction also matters. Public relays impose their own limits on the ASP's outbound writes: max events per pubkey per minute, max payload size, max subscriptions per connection, sometimes proof of work. None of these are standardized; each operator picks. Round coordination tends to be bursty (tree nonces, tree signatures, finalized tx, batch confirmations published in a short window), so an ASP riding on public relays tuned for typical social traffic will trip limits and see writes silently dropped. This is one of the operational pressures that pushed NWC onto provider operated relays. ASPs running Ark traffic on public relays will need to vet each relay's limits, and the realistic deployment is an ASP operated or partner operated relay set where the operator controls the policy.
...
* **Realistic deployment shape.** Following the NWC precedent (see §4.3 practical note), an ASP serving Ark traffic will likely run its own relay or partner with operators who do, rather than relying on the open commons of public Nostr relays. This gives the operator control over rate limits, retention, and accepted kinds, at the cost of some of the "any relay works" framing the Nostr layer suggests. The censorship resistance argument is meaningful but bounded: an ASP operated relay set is still much faster to rotate than a TLS endpoint (one signed kind 10002 event vs. DNS plus cert plus deploy), but it is not "all of Nostr."
* **Async transport benefits beyond censorship resistance.** Routing client traffic through relays rather than a fixed gRPC endpoint is also beneficial for horizontal scalability and microservice architecture, enabling high availability across multiple datacenters. The Nostr layer removes the connection level pinning that long lived gRPC streams create, so an ASP can scale out or fail over without forcing wallets to reconnect.
...
* **Cold start when every known relay is blocked.** NIP 65 lets a wallet rotate among relays it already knows, but if all of those are blocked at once, the wallet has no in protocol way to learn a fresh one. Recovery falls back to out of band channels: a tweet from the ASP, a fresh app update, a friend sharing the address. The new relay address then carries the same trust question covered in §7.6 (first contact trust), just at the relay layer instead of the ASP layer. Nostr does not solve this fully; the proposal raises the cost of censorship over a single TLS endpoint but does not eliminate it.
